...
bulk = subparsers.add_parser("bulk_execute", help="Bulk execute commands from a txt file")
...
# Command: maesy bulk_execute
bulk.add_argument("queue_file", type=str, help="Path to the txt file containing commands to execute. Either an absolute path, or a filename in cfg")
...
dataset_convert_parser.add_argument("--convert-permute-ids", type=int, nargs='+', default=[], help="Space-separated list of indices to permute the class IDs when converting/creating the dataset. Only works when --convert flag is used as well.")
...
def dispatch_command(argv: list[str]) -> int:
    ...
    match args.module:
        ...
        case _:
            raise NotImplementedError(f"Module '{args.module}' not recognized.")
