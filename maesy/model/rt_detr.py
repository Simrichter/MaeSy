...
class RTDETRBackbone(nn.Module):
    """Multi-scale ResNet backbone that exposes C3/C4/C5 features."""

    def __init__(self, config: RTDETRBackboneConfig):
        super().__init__()
        self.config = config
        self.type = f"RTDETRBackbone_{config.version}"
        ...
        self.feature_channels = self._get_feature_channels(config.version)

    @staticmethod
    def _get_feature_channels(version: str) -> Tuple[int, int, int]:
        if version in {"resnet18", "resnet34"}:
            return 128, 256, 512
...
class RTDETRHead(nn.Module):
    ...
    def _hybrid_encode(self, features: List[torch.Tensor]) -> List[torch.Tensor]:
        p3, p4, p5 = [proj(feat) for proj, feat in zip(self.input_proj, features)]
...
class RTDETR(BaseModel):
    ...
    def __init__(self, config: RTDETRConfig):
        ...
        self.backbone = RTDETRBackbone(
            RTDETRBackboneConfig(
                version=config.resnet_version,
        ...
        print(
            ...
            f"\n Feature channels: {self.backbone.feature_channels}"
        )
