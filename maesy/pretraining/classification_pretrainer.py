...
class ClassificationPretrainer:
    ...
    def __init__(
        ...
        config: Optional[ClassificationPretrainingConfig] = None
    ):
        """
        Initialize classification pretrainer.
        
        Args:
            model: Classification model to pretrain
        ...
        self.config = config or ClassificationPretrainingConfig()
    ...
    def train(self) -> None:
        """Main training loop."""
        print(f"Starting classification pretraining for {self.config.num_epochs} epochs")
        ...
        for epoch in range(self.config.num_epochs):
            ...
            if self.val_loader is not None:
                ...
                print(f"Epoch {epoch + 1}/{self.config.num_epochs} - "
                      ...
                      f"Val Loss: {val_metrics['val_loss']:.4f}, Val Acc: {val_metrics['val_accuracy']:.2f}%")
                
                # Save best model based on validation accuracy
                if val_metrics['val_accuracy'] > self.best_val_acc:
                    self.best_val_acc = val_metrics['val_accuracy']
                    self.best_val_loss = val_metrics['val_loss']
                    self.save_checkpoint('classification_best_model.pth')
            else:
                print(f"Epoch {epoch + 1}/{self.config.num_epochs} - "
                      f"Train Loss: {train_metrics['loss']:.4f}, Train Acc: {train_metrics['accuracy']:.2f}%")
            ...
            if (epoch + 1) % self.config.save_frequency == 0:
                self.save_checkpoint(f'classification_checkpoint_epoch_{epoch + 1}.pth')
